...
class GradientDescent:
    ...
    def __init__(self, loss, regularization=None,
        ...
        self.model = None
        self.model_loss_list = []
        self.model_accuracy_list = []

    def get_model_performance(self):
        return self.model_accuracy_list, self.model_loss_list
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        if batch_size:
            ...
            epoch = features.shape[0]//batch_size

            while abs(current_loss - previous_loss) > 1e-4 and iterations < max_iter:

                for i in range(epoch):

                    current_loss = self.loss.forward(
                        features, self.model, targets)

                    self.model_loss_list.append(current_loss)

                    self.model_accuracy_list.append(
                        accuracy(targets, self.predict(static_features)))

                    previous_loss = current_loss

                    self.model = self.model - self.learning_rate * \
                        self.loss.backward(features, self.model, targets)
    ...
    def predict(self, features):
        ...
        """
        list_predict = []
        predict = self.confidence(features)

        for feature in predict:
            if feature > 0:
                output = 1
                list_predict.append(output)
            else:
                output = -1
                list_predict.append(output)

        predictions = np.array(list_predict)
    ...
    def confidence(self, features):
        ...
        #confidence = np.array([np.dot(self.model, feature)])

        return features.dot(self.model)
        #raise NotImplementedError()
