...
class L1Regularization(Regularization):
    ...
    def forward(self, w):
        ...
        regularization_term = self.reg_param * np.sum(np.abs(w[:-1]))
    ...
    def backward(self, w):
        ...
        Returns:
            gradient_term - (np.array) A numpy array of length d+1. The
                gradient of the regularization term evaluated at w.
        """
        gradient_term = self.reg_param * \
            np.array([1 if param > 0 else 0 if param ==
                     0 else -1 for param in w[:-1]])
...
class L2Regularization(Regularization):
    ...
    def forward(self, w):
        ...
        regularization_term = 1/2 * self.reg_param * np.sum(np.abs(w[:-1])**2)
    ...
    def backward(self, w):
        ...
        gradient_term = self.reg_param * np.append(w[:-1], 0)
